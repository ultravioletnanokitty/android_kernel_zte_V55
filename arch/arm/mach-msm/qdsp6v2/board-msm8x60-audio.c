...
static void config_class_enb_en_gpio20(int enable)
{
	...
	if (enable) {
		rc = gpio_request(SNDDEV_ENB_EN_GPIO20, "GPIO20_ENB_EN");
		if (rc) {
			pr_err("%s: spkr pamp gpio %d request"
			"failed\n", __func__, SNDDEV_ENB_EN_GPIO20);
			return;
		}
		gpio_direction_output(SNDDEV_ENB_EN_GPIO20, 1);
		gpio_set_value_cansleep(SNDDEV_ENB_EN_GPIO20, 1);
	} else {
		gpio_set_value_cansleep(SNDDEV_ENB_EN_GPIO20, 0);
		gpio_free(SNDDEV_ENB_EN_GPIO20);
	}
}
