...
static void led_update_blink(struct led_classdev *led_cdev)
{
	if (led_cdev->blink_get)
		led_cdev->blink = led_cdev->blink_get(led_cdev);
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ssize_t ret = -EINVAL;
	char *after;
	unsigned long pwm = simple_strtoul(buf, &after, 10);
	size_t count = after - buf;
