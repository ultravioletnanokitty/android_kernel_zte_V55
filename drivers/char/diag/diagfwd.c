...
static int diag_process_apps_pkt(unsigned char *buf, int len)
{
	...
	/* Check for our extended download command */
	else if (cpu_is_msm8x60() && (*buf == DIAG_DLOAD_F_EX)) {
		/* send response back */
		driver->apps_rsp_buf[0] = *buf;
		ENCODE_RSP_AND_SEND(0);
		msleep(5000);
		/* call download API */
		msm_set_restart_mode(RESTART_DLOAD_EX);
...
{
	printk("sprint_diag!sunwei!cmd_code=0x%x,subsys_id=0x%x!",cmd_code,subsys_id);
	switch(subsys_id)
	{
		case 0x32:// State&ConnectionAttempts
		//case 0x39:// RSSI
		{
			printk("sprint_diag!sunwei!Force jump to Modem!");
			return 1;
		}
		default:
		{
