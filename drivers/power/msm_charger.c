				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		//val->intval = get_prop_battery_mvolts(); //huxb fixed for ADC voltage, 2011.01.25
		val->intval = max17040_get_vcell(max17040_bak_client) * 1000;
		break;
	case POWER_SUPPLY_PROP_CAPACITY:
		//val->intval = get_prop_batt_capacity();
		val->intval = max17040_get_soc(max17040_bak_client);  //huxb fixed, 2011.02.06
		
		if ((val->intval < 3) && (max17040_get_vcell(max17040_bak_client) < 3400)){
			val->intval = 0;
		}else if (val->intval == 0){
			val->intval = BATTERY_LOW_SOC_WARNING;
		}	
		
        if ((val->intval == 0) && (msm_chg.current_chg_priv != NULL)){
            val->intval = BATTERY_LOW_SOC_WARNING;
        }
		break;
	default:
...
static int msm_psy_batt_registered = 0;
...
static void (*get_usb_charger_state)(int *mA);
/*
 * msm_register_usb_charger_state() - set the get_usb_charger_state call back function
 * @function:	function to register 
 */
void msm_register_usb_charger_state(void (*function)(int *))
{
 ...
 * @mA	  : Current value
 */
void msm_set_source_current(struct msm_hardware_charger *hw_chg, unsigned int mA)
{
	struct msm_hardware_charger_priv *priv;
	
	priv = hw_chg->charger_private;
	priv->max_source_current = mA;

	pr_debug("%s:%s source current set to %dmA\n", __func__, hw_chg->name, mA);
}
#endif
