...
extern int dock_audio_insert_set(bool insert);

#define ZTE_DOCK_HEADSET_DET 73
...
static enum hrtimer_restart dock_audio_det_timer(struct hrtimer *timer)
{
	int rc;
	struct pm8058_othc *dd = container_of(timer,
					struct pm8058_othc, dock_timer);

	rc = gpio_get_value_cansleep(ZTE_DOCK_HEADSET_DET);
	printk("wangweiping test dock dd->dock_insert=%d, rc = %d\n", dd->dock_insert, rc);
...
othc_configure_hsed(struct pm8058_othc *dd, struct platform_device *pd)
{
	...
	rc = gpio_direction_input(ZTE_DOCK_HEADSET_DET);
	if (rc) {
		pr_err("GPIO %d set_direction failed\n", dd->ir_gpio);
