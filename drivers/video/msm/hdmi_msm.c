...
static void hdmi_msm_hpd_read_work(struct work_struct *work)
{
	...
	hdmi_msm_state->pd->enable_level_shift(1);//gouyajun0002
	hdmi_msm_state->pd->enable_dc_dc(1);//gouyajun0002
	hdmi_msm_set_mode(FALSE);
	hdmi_msm_init_phy(external_common_state->video_resolution);
	/* HDMI_USEC_REFTIMER[0x0208] */
	HDMI_OUTP(0x0208, 0x0001001B);
	...
	hdmi_msm_state->pd->enable_level_shift(0);//gouyajun0002
	hdmi_msm_state->pd->enable_dc_dc(0);//gouyajun0002
	clk_disable(hdmi_msm_state->hdmi_app_clk);
}
...
static int __devinit hdmi_msm_probe(struct platform_device *pdev)
{
	...
	if (!hdmi_msm_state->pd->enable_level_shift) {//gouyajun0002
		DEV_ERR("Init FAILED: enable level shifr  function missing\n");
		rc = -ENODEV;
		goto error;
	}
	if(!hdmi_msm_state->pd->enable_dc_dc){//gouyajun0002
		DEV_ERR("Init FAILED: enable dc dc  function missing\n");
		rc = -ENODEV;
		goto error;
	}	
...
static int hdmi_msm_device_pm_suspend(struct device *dev)
{
	...
	hdmi_msm_powerdown_phy();
	hdmi_msm_state->pd->enable_5v(0);
	hdmi_msm_state->pd->enable_level_shift(0);//gouyajun20110523
	hdmi_msm_state->pd->enable_dc_dc(0);//gouyajun0002
	hdmi_msm_state->pd->core_power(0, 1);
	return 0;
}
...
static int hdmi_msm_device_pm_resume(struct device *dev)
{
	...
	hdmi_msm_state->pd->core_power(1, 1);
	hdmi_msm_state->pd->enable_5v(1);
	hdmi_msm_state->pd->enable_level_shift(1);//gouyajun20110523
	hdmi_msm_state->pd->enable_dc_dc(1);//gouyajun0002
	if (external_common_state->hpd_feature_on)
		hdmi_msm_clk(1);
