...
static inline void l2cap_do_send(struct sock *sk, struct sk_buff *skb)
{
	...
	if(!pi ||!(pi->conn)) {
		printk("chengjiatest: l2cap conn is NULL\n");
		return;
	}
...
static int l2cap_ertm_send_txq(struct sock *sk)
{
	...
	if(sk_ptr[0] == (unsigned long)sk || sk_ptr[1] == (unsigned long)sk) {
		if (printk_ratelimit()) {
	...
	l2cap_pi(sk)->trans_flag = 1;
	BT_DBG("sk %p", sk);

	if (sk->sk_state != BT_CONNECTED) {
		l2cap_pi(sk)->trans_flag = 0;
		return -ENOTCONN;
	}
